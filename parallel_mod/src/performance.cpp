...
std::vector<measurement> run_experiments()
{
	...
	const auto data = std::make_unique<IntegerWord[]>(word_count);
	...
	for (unsigned T = 1; T <= std::thread::hardware_concurrency(); ++T)
	{
		...
		const auto result = vector_mod(data.get(), word_count, divisor);
		const auto time = duration_cast<milliseconds>(steady_clock::now() - tm0);
